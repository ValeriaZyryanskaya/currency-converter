...
class Vue {
    ...
    render(lbtn,rbtn,linp,rinp){
        ...
        this.page.left.hint.innerText = `1 ${lbtn} = ${(linp/rinp).toFixed(2)} ${rbtn}`
        this.page.right.hint.innerText = `1 ${rbtn} = ${(rinp/linp).toFixed(2)} ${lbtn}`
    }
    ...
    getCallBackFromInput(direction) {
        return (e)=>{
            if (direction=="left"){
                ...
                this.rInp=(this.lInp*this.coeff).toFixed(2);
                this.toRender();
            }else{
                this.rInp=isNaN(e.target.value) ? this.rInp : e.target.value;
                this.lInp=(this.rInp/this.coeff).toFixed(2);
                this.toRender();
            }
